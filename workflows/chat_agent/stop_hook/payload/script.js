/**
 * telegram.services/chat_agent/stop_hook.js - preflight/postflight functions for telegram.services/chat_agent/stop_hook
 * @module telegram.services/chat_agent/stop_hook
 * @file telegram.services/chat_agent/stop_hook preflight/postflight implementation
 * @author system
 * @copyright Copyright ©2024, Concluent Systems, LLC. All rights reserved.
 */
"use strict";
const MODULE_NAME = "workflow:telegram.services/chat_agent/stop_hook";
...
function postflight(authData, wfProxy) {
    ...
    try {
        return bot.bot.stop ().then (res => {
            ...
            delete global.REBAR_NAMESPACE.__telegram [ta_alias];
            return Promise.resolve ({success:true});
        });
    }
    catch (err) {
